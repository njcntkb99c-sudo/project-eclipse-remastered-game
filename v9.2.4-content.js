...
$("versionLabel").textContent="Reborn v9.2.4 · Navigation HUD";
...
function draw(){ctx.drawImage(images.sky,0,0);let a=(cameraX*.10)%960;ctx.drawImage(images.far,-a,0);ctx.drawImage(images.far,960-a,0);let b=(cameraX*.22)%960;ctx.drawImage(images.mid,-b,0);ctx.drawImage(images.mid,960-b,0);drawRain();ctx.save();if(shake>0){ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);shake*=.84;if(shake<.4)shake=0}ctx.translate(-cameraX,0);drawPropLayer("back");platforms.forEach((p,i)=>{
const ts=48,cols=16;
