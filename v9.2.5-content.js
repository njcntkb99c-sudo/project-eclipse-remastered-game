...
$("versionLabel").textContent="Reborn v9.2.5 · Stability Build";
...
let assetsReady=false,victory=false,noticeTimer=0,shake=0,comboCount=0,comboTimer=0,rainTime=0,fogTime=0,hitStop=0;
...
function save(){try{localStorage.setItem("eclipseV9",JSON.stringify({difficulty,souls,deaths,checkpoint,relicCount,level,xp,nextXp}));ui.continueGame.disabled=false}catch{notice("SALVATAGGIO NON DISPONIBILE")}}
...
function damage(n,x){if(P.inv>0||mode!=="play")return;if(P.block>0&&Math.sign(x-P.x)===P.facing){P.st=Math.max(0,P.st-18);P.inv=.22;burst(P.x+38,P.y+40,"#b8dcff",12);notice("PARATA");return}hitStop=.055;P.hp-=n;P.inv=1;P.vx=(P.x<x?-1:1)*330;P.vy=-250;shake=9;burst(P.x+30,P.y+45,"#c33b53",18);if(P.hp<=0)die()}
function kill(e){e.dead=true;souls+=e.type==="boss"?250:14;gainXp(e.type==="boss"?250:30);if(e.type==="boss"){victory=true;mode="victory";notice("IL CUSTODE DEL PATTO È CADUTO");save()}}
function attack(){if(P.attackCd>0||mode!=="play")return;if(P.comboWindow>0)P.comboStep=(P.comboStep%3)+1;else P.comboStep=1;P.comboWindow=.56;P.attack=.31;P.attackCd=.29;let reach=[0,80,92,110][P.comboStep],base=[0,46,55,72][P.comboStep],hit={x:P.facing>0?P.x+P.w:P.x-reach,y:P.y+17,w:reach,h:70};enemies.forEach(e=>{if(e.dead||!overlap(hit,e))return;let dmg=e.type==="boss"?Math.round((base+P.baseDamage-40)*.76):base+P.baseDamage-40;e.hp-=dmg;e.hurt=.18;e.vx=P.facing*(195+P.comboStep*32);shake=P.comboStep===3?10:5;burst(e.x+e.w/2,e.y+e.h/2,P.comboStep===3?"#d7efff":"#72baff",15+P.comboStep*4);comboCount++;comboTimer=2;ui.comboFlash.querySelector("b").textContent=comboCount;ui.comboFlash.classList.add("on");hitStop=P.comboStep===3?.075:.035;if(e.hp<=0)kill(e)})}
...
function draw(){ctx.drawImage(images.sky,0,0);let a=(cameraX*.10)%960;ctx.drawImage(images.far,-a,0);ctx.drawImage(images.far,960-a,0);let b=(cameraX*.22)%960;ctx.drawImage(images.mid,-b,0);ctx.drawImage(images.mid,960-b,0);drawRain();ctx.save();if(shake>0){ctx.translate((Math.random()-.5)*shake,(Math.random()-.5)*shake);shake*=.84;if(shake<.4)shake=0}ctx.translate(-cameraX,0);drawPropLayer("back");platforms.forEach((p,i)=>{if(p.x+p.w<cameraX-96||p.x>cameraX+canvas.width+96)return;
const ts=48,cols=16;
