...
$("versionLabel").textContent="Reborn v9.5.3 · NPC Dialogues";
...
drawWorldLights();drawNpcs();drawLoreFragments();drawEliteAuras();drawDamageTexts();
...
const npcDatabase={"npcs": [{"id": "mara", "name": "Mara, Custode delle Candele", "x": 1180, "dialogue": {"start": {"text": "Le tombe ricordano ciò che i vivi scelgono di dimenticare. Cerchi il Custode?", "choices": [{"label": "Chi era il Custode?", "next": "warden"}, {"label": "Hai bisogno di aiuto?", "next": "quest"}, {"label": "Lascia stare.", "next": "end"}]}, "warden": {"text": "Un cavaliere fedele. Troppo fedele. Quando il re spezzò il patto, lui continuò a difenderne le rovine.", "choices": [{"label": "Capisco.", "next": "end"}]}, "quest": {"text": "Tre candele votive sono state spente nelle rovine. Riaccendile e tornerò a fidarmi dei viandanti.", "choices": [{"label": "Accetto.", "action": "accept_candles", "next": "end"}, {"label": "Non ora.", "next": "end"}]}, "end": {"text": "Che l'Essenza non confonda i tuoi ricordi.", "choices": []}}}]};
const npcState={active:null,node:"start",quests:{candles:{accepted:false,progress:0,required:3,complete:false,rewarded:false}}};
const dialoguePanel=document.createElement("div");dialoguePanel.id="dialoguePanel";dialoguePanel.hidden=true;
dialoguePanel.innerHTML='<div class="dialogue-card"><h3 id="dialogueName"></h3><p id="dialogueText"></p><div id="dialogueChoices"></div></div>';
document.querySelector(".game-box").appendChild(dialoguePanel);
function nearestNpc(){return npcDatabase.npcs.find(n=>Math.abs(P.x-n.x)<95)}
function closeDialogue(){dialoguePanel.hidden=true;npcState.active=null;npcState.node="start"}
function runDialogueAction(action){
 if(action==="accept_candles"){npcState.quests.candles.accepted=true;notice("QUEST: LE CANDELE SPENTE")}
}
function showDialogue(){
 const npc=npcState.active,node=npc.dialogue[npcState.node];
 dialoguePanel.hidden=false;document.getElementById("dialogueName").textContent=npc.name;
 document.getElementById("dialogueText").textContent=node.text;
 const choices=document.getElementById("dialogueChoices");choices.innerHTML="";
 if(!node.choices.length){const b=document.createElement("button");b.textContent="Chiudi";b.onclick=closeDialogue;choices.appendChild(b);return}
 node.choices.forEach(c=>{const b=document.createElement("button");b.textContent=c.label;b.onclick=()=>{if(c.action)runDialogueAction(c.action);npcState.node=c.next;showDialogue()};choices.appendChild(b)});
}
document.addEventListener("keydown",e=>{
 if(e.code==="KeyE"&&mode==="play"&&dialoguePanel.hidden){
  const npc=nearestNpc();if(npc){npcState.active=npc;npcState.node="start";showDialogue()}
 }
});
function drawNpcs(){ctx.save();npcDatabase.npcs.forEach(n=>{ctx.fillStyle="#8d7b65";ctx.fillRect(n.x,GROUND-72,28,72);ctx.fillStyle="#e6c27f";ctx.fillRect(n.x+8,GROUND-86,12,14);ctx.fillStyle="#b9dff3";ctx.font="11px monospace";ctx.fillText("E",n.x+9,GROUND-98)});ctx.restore()}
})();
